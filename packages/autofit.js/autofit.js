...
let curElRectification = '';
let curElRectificationLevel = '';
    ...
    init(options = {}, isShowInitTip = true) {
        ...
        resizeListener = () => {
            ...
            if (delay != 0)
                timer = setTimeout(() => {
                    keepFit(dw, dh, dom, ignore);
                    isElRectification && elRectification(curElRectification, curElRectificationLevel);
                }, delay);
            else {
                keepFit(dw, dh, dom, ignore);
                isElRectification && elRectification(curElRectification, curElRectificationLevel);
...
function elRectification(el, level = 1) {
    ...
    curElRectification = el;
    curElRectificationLevel = level;
...
function offElRectification() {
    if (!curElRectification) return;
    for (let item of document.querySelectorAll(curElRectification)) {
        item.style.width = ``;
        item.style.height = ``;
